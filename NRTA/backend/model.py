...
class NVNet(torch.nn.Module):
    ...
    class Decoder(nn.Module):
        ...
        def forward(self, enc_8, enc_16, enc_32, enc_64, enc_128):
            dec_64 = self.BN_64(self.activ(self.Deconv_128_64(enc_128)))
            dec_32 = self.BN_32(self.activ(self.Deconv_64_32(torch.cat([enc_64, dec_64], dim=1))))
            dec_16 = self.BN_16(self.activ(self.Deconv_32_16(torch.cat([enc_32, dec_32], dim=1))))
            dec_8 = self.BN_8(self.activ(self.Deconv_16_8(torch.cat([enc_16, dec_16], dim=1))))
    ...
    def forward(self, x):
        ...
        m_vec = torch.cat([dec_x, dec_y, torch.zeros_like(dec_y)], dim=1)
        return self.amp*m_vec
